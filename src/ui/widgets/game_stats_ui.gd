...
func _on_drowned_with_reward(reward_value):
	rewards_label.text = str(int(rewards_label.text) + reward_value)
#endregion
...
func _ready() -> void:
	...
	EventBus.on_drowned_with_reward.connect(_on_drowned_with_reward)
